...
class DisclosureToUAServiceSpec extends AnyWordSpec with Matchers with TryValues {
  ...
  "otherLiabilitiesToUa" should {
    ...
    "return a PageWithValue for all that are set" in {
      val otherIssuesSet: Set[OtherLiabilityIssues] =
        Set(OtherLiabilityIssues.Other, OtherLiabilityIssues.InheritanceTaxIssues)
      val otherLiabilities                          = OtherLiabilities(
        Some(otherIssuesSet),
        Some("Some gift"),
        Some("Some issue"),
  ...
  "offshoreLiabilitiesToUa" should {
    ...
    "return a PageWithValue for all that are set" in {
      ...
      updatedUserAnswers.get(WhatReasonableCareDidYouTakePage)                 shouldEqual Some(
        WhatReasonableCareDidYouTake("Some excuse", "Some years")
      )
      updatedUserAnswers.get(WhatIsYourReasonableExcuseForNotFilingReturnPage) shouldEqual Some(
        WhatIsYourReasonableExcuseForNotFilingReturn("Some excuse", "Some years")
      )
      ...
      updatedUserAnswers.get(TaxYearLiabilitiesPage)                           shouldEqual Some(
        Map("2012" -> TaxYearWithLiabilities(TaxYearStarting(2012), liabilities))
      )
      updatedUserAnswers.get(ForeignTaxCreditPage)                             shouldEqual Some(Map("2012" -> BigInt(123)))
      updatedUserAnswers.get(CountryOfYourOffshoreLiabilityPage)               shouldEqual Some(
        Map("GBR" -> Country("GBR", "United Kingdom"))
      )
      ...
      updatedUserAnswers.get(HowMuchTaxHasNotBeenIncludedPage)                 shouldEqual Some(
  ...
  "reasonForDisclosingNowToUa" should {
    ...
    "return a PageWithValue for all that are set" in {
      ...
      updatedUserAnswers.get(AdviceGivenPage)                             shouldEqual Some(
        AdviceGiven("Some advice", MonthYear(12, 2012), AdviceContactPreference.No)
      )
      updatedUserAnswers.get(WhichEmailAddressCanWeContactYouWithPage)    shouldEqual Some(
        WhichEmailAddressCanWeContactYouWith.values.head
      )
      updatedUserAnswers.get(WhichTelephoneNumberCanWeContactYouWithPage) shouldEqual Some(
        WhichTelephoneNumberCanWeContactYouWith.values.head
      )
  ...
  "initialiseUserAnswers" should {
    "retrieve the userId, submissionId and lastUpdated from the notification" in {
      ...
      val expectedResult = UserAnswers(
        ...
        submissionId = "Some notification Id",
  ...
  "fullDisclosureToUa" should {
    "populate UA with everything within the FullDisclosure object" in {
      ...
      val whyOnlineSet: Set[WhyAreYouMakingThisOnshoreDisclosure] =
  ...
  "onshoreLiabilitiesToUa" should {
    ...
    "return a PageWithValue for all that are set" in {
      ...
      updatedUserAnswers.get(ReasonableCareOnshorePage)                     shouldEqual Some(
        ReasonableCareOnshore("Some excuse", "Some years")
      )
      updatedUserAnswers.get(ReasonableExcuseForNotFilingOnshorePage)       shouldEqual Some(
        ReasonableExcuseForNotFilingOnshore("Some excuse", "Some years")
      )
      ...
      updatedUserAnswers.get(OnshoreTaxYearLiabilitiesPage)                 shouldEqual Some(
        Map("2012" -> OnshoreTaxYearWithLiabilities(OnshoreYearStarting(2012), liabilities))
      )
  ...
  "migrate legacy onshore behaviour to Page 1 and Page 2 values" should {
    ...
    "migrate didNotNotifyNoExcuse to DidNotNotifyHMRC and NotDeliberatelyNoReasonableExcuseOnshore" in {
      ...
      val updatedUserAnswers = sut.onshoreLiabilitiesToUa(Some(onshoreLiabilities), emptyUA).success.value
      updatedUserAnswers.get(WhyAreYouMakingThisOnshoreDisclosurePage)     shouldEqual Some(
        Set(WhyAreYouMakingThisOnshoreDisclosure.DidNotNotifyHMRC)
      )
      updatedUserAnswers.get(WhyDidYouNotNotifyOnshorePage)                shouldEqual Some(
        Set(WhyDidYouNotNotifyOnshore.NotDeliberatelyNoReasonableExcuseOnshore)
      )
    ...
    "migrate didNotNotifyHasExcuse to DidNotNotifyHMRC and ReasonableExcuseOnshore" in {
      ...
      updatedUserAnswers.get(WhyDidYouNotNotifyOnshorePage)                shouldEqual Some(
        Set(WhyDidYouNotNotifyOnshore.ReasonableExcuseOnshore)
      )
      updatedUserAnswers.get(WhyDidYouNotFileAReturnOnTimeOnshorePage)     shouldEqual None
      updatedUserAnswers.get(WhyYouSubmittedAnInaccurateOnshoreReturnPage) shouldEqual None
    }
    ...
    "migrate deliberatelyDidNotNotify to DidNotNotifyHMRC and DeliberatelyDidNotNotifyOnshore" in {
      ...
      updatedUserAnswers.get(WhyDidYouNotNotifyOnshorePage)            shouldEqual Some(
        Set(WhyDidYouNotNotifyOnshore.DeliberatelyDidNotNotifyOnshore)
      )
    }
    ...
    "migrate didNotFileNoExcuse to DidNotFile and DidNotWithholdInformationOnPurpose" in {
      ...
      val updatedUserAnswers = sut.onshoreLiabilitiesToUa(Some(onshoreLiabilities), emptyUA).success.value
      updatedUserAnswers.get(WhyAreYouMakingThisOnshoreDisclosurePage)     shouldEqual Some(
        Set(WhyAreYouMakingThisOnshoreDisclosure.DidNotFile)
      )
      updatedUserAnswers.get(WhyDidYouNotFileAReturnOnTimeOnshorePage)     shouldEqual Some(
        Set(WhyDidYouNotFileAReturnOnTimeOnshore.DidNotWithholdInformationOnPurpose)
      )
    ...
    "migrate notFileHasExcuse to DidNotFile and ReasonableExcuse" in {
      ...
      updatedUserAnswers.get(WhyDidYouNotFileAReturnOnTimeOnshorePage) shouldEqual Some(
        Set(WhyDidYouNotFileAReturnOnTimeOnshore.ReasonableExcuse)
      )
    }
    ...
    "migrate inaccurateReturnNoCare to InaccurateReturn and NoReasonableCare" in {
      ...
      val updatedUserAnswers = sut.onshoreLiabilitiesToUa(Some(onshoreLiabilities), emptyUA).success.value
      updatedUserAnswers.get(WhyAreYouMakingThisOnshoreDisclosurePage)     shouldEqual Some(
        Set(WhyAreYouMakingThisOnshoreDisclosure.InaccurateReturn)
      )
      updatedUserAnswers.get(WhyYouSubmittedAnInaccurateOnshoreReturnPage) shouldEqual Some(
  ...
  "migrate legacy offshore behaviour to Page 1 and Page 2 values" should {
    ...
    "migrate didNotNotifyNoExcuse to DidNotNotifyHMRC and NotDeliberatelyNoReasonableExcuse" in {
      ...
      val updatedUserAnswers  = sut.offshoreLiabilitiesToUa(offshoreLiabilities, emptyUA).success.value
      updatedUserAnswers.get(WhyAreYouMakingThisDisclosurePage)             shouldEqual Some(
        Set(WhyAreYouMakingThisDisclosure.DidNotNotifyHMRC)
      )
      updatedUserAnswers.get(WhyDidYouNotNotifyPage)                        shouldEqual Some(
        Set(WhyDidYouNotNotify.NotDeliberatelyNoReasonableExcuse)
      )
    ...
    "migrate didNotFileNoExcuse to DidNotFile and DidNotWithholdInformationOnPurpose" in {
      ...
      updatedUserAnswers.get(WhyDidYouNotFileAReturnOnTimeOffshorePage) shouldEqual Some(
        Set(WhyDidYouNotFileAReturnOnTimeOffshore.DidNotWithholdInformationOnPurpose)
      )
    ...
    "migrate deliberateInaccurateReturn to InaccurateReturn and DeliberatelyInaccurate" in {
      ...
      updatedUserAnswers.get(WhyYouSubmittedAnInaccurateOffshoreReturnPage) shouldEqual Some(
        Set(WhyYouSubmittedAnInaccurateReturn.DeliberatelyInaccurate)
      )
    }
  ...
  "OnshoreLiabilities JSON deserialisation" should {
    ...
    "deserialise multiple legacy values from JSON" in {
      ...
      val result = json.validate[OnshoreLiabilities]
      result.isSuccess     shouldEqual true
      result.get.behaviour shouldEqual Some(
        Set(
          WhyAreYouMakingThisOnshoreDisclosure.DidNotNotifyNoExcuse,
          WhyAreYouMakingThisOnshoreDisclosure.DidNotFileNoExcuse
        )
