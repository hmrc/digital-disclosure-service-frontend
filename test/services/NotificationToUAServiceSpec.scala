...
class NotificationToUAServiceSpec extends AnyWordSpec with Matchers with TryValues {
  ...
  "entityPagesWithValues" should {
    ...
    "return the pagesWithValues for a Trust who has answered if they are the Trust" in {
      ...
      val expectedResult   =
        List(PageWithValue(RelatesToPage, RelatesTo.ATrust), PageWithValue(AreYouTheEntityPage, AreYouTheEntity.YesIAm))
      sut.entityPagesWithValues(disclosureEntity) shouldEqual expectedResult
    }
    ...
    "return the pagesWithValues for an Estate who has answered if they are an executor" in {
      ...
      val expectedResult   = List(
        PageWithValue(RelatesToPage, RelatesTo.AnEstate),
        PageWithValue(AreYouTheEntityPage, AreYouTheEntity.YesIAm)
      )
      sut.entityPagesWithValues(disclosureEntity) shouldEqual expectedResult
    }
  ...
  "aboutYouToUserAnswers" should {
    ...
    "return a PageWithValue for all that are set" in {
      ...
      updatedUserAnswers.get(DoYouHaveNationalInsuranceNumberPage)  shouldEqual Some(
      ...
      updatedUserAnswers.get(AreYouRegisteredForSelfAssessmentPage) shouldEqual Some(
        AreYouRegisteredForSelfAssessment.No
      )
  ...
  "aboutTheIndividualToUserAnswers" should {
    ...
    "return a PageWithValue for all that are set" in {
      ...
      updatedUserAnswers.get(DoesTheIndividualHaveNationalInsuranceNumberPage) shouldEqual Some(
      ...
      updatedUserAnswers.get(IsTheIndividualRegisteredForVATPage)              shouldEqual Some(
      ...
      updatedUserAnswers.get(IsTheIndividualRegisteredForSelfAssessmentPage)   shouldEqual Some(
        IsTheIndividualRegisteredForSelfAssessment.No
      )
  ...
  "aboutTheEntityToUserAnswers" should {
    ...
    "return populated AboutTheIndividual information where the entity is set to Individual" in {
      val background      = Background(disclosureEntity =
        Some(DisclosureEntity(Individual, Some(AreYouTheEntity.IAmAnAccountantOrTaxAgent)))
      )
      val personalDetails = testNotification.personalDetails
        .copy(background = background, aboutTheIndividual = Some(AboutTheIndividual(fullName = Some("Some full name"))))
    ...
    "return populated AboutTheCompany information where the entity is set to Company" in {
      val background      =
        Background(disclosureEntity = Some(DisclosureEntity(Company, Some(AreYouTheEntity.IAmAnAccountantOrTaxAgent))))
      val personalDetails = testNotification.personalDetails
        .copy(background = background, aboutTheCompany = Some(AboutTheCompany(name = Some("Some name"))))
    ...
    "return populated AboutTheTrust information where the entity is set to Trust" in {
      val background      =
        Background(disclosureEntity = Some(DisclosureEntity(Trust, Some(AreYouTheEntity.IAmAnAccountantOrTaxAgent))))
      val personalDetails = testNotification.personalDetails
        .copy(background = background, aboutTheTrust = Some(AboutTheTrust(name = Some("Some name"))))
    ...
    "return populated AboutTheLLP information where the entity is set to LLP" in {
      val background      =
        Background(disclosureEntity = Some(DisclosureEntity(LLP, Some(AreYouTheEntity.IAmAnAccountantOrTaxAgent))))
      val personalDetails = testNotification.personalDetails
        .copy(background = background, aboutTheLLP = Some(AboutTheLLP(name = Some("Some name"))))
  ...
  "aboutTheEstateToUserAnswers" should {
    ...
    "return a PageWithValue for all that are set" in {
      ...
      updatedUserAnswers.get(WasThePersonRegisteredForVATPage)          shouldEqual Some(
        WasThePersonRegisteredForVAT.YesButIDontKnow
      )
