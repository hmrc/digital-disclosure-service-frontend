...
class WhyAreYouMakingADisclosureViewSpec extends ViewSpecBase with ViewMatchers {
  ...
  "view" should {
    ...
    "have title" in {
      view.select("title").text() must include(messages("whyAreYouMakingADisclosure.title"))
    }

    "contain header" in {
      view.getElementsByClass("govuk-heading-xl").text() mustBe messages("whyAreYouMakingADisclosure.heading")
    }

    "contain body" in {
      view.getElementById("paragraph").text() mustBe messages("whyAreYouMakingADisclosure.body")
    }
