...
class PropertyAddedViewSpec extends ViewSpecBase with ViewMatchers {
  ...
  "view" should {
    ...
    "have title" in {
      view.select("title").text() must include(messages("propertyAdded.title"))
    }

    "contain header" in {
      view.getElementsByClass("govuk-fieldset__heading").text() mustBe messages("propertyAdded.heading")
    }
