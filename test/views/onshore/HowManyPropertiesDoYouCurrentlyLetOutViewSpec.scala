...
class HowManyPropertiesDoYouCurrentlyLetOutViewSpec extends ViewSpecBase with ViewMatchers {
  ...
  "view" should {
    ...
    "have title" in {
      view.select("title").text() must include(messages("howManyProperties.title"))
    }

    "contain header" in {
      view.getElementsByClass("govuk-label--xl").text() mustBe messages("howManyProperties.heading")
    }
