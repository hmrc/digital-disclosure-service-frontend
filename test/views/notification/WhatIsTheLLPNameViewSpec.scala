...
class WhatIsTheLLPNameViewSpec extends ViewSpecBase with ViewMatchers {
  ...
  "view" should {
    ...
    "have title" in {
      view.select("title").text() must include(messages("whatIsTheLLPName.title"))
    }

    "contain header" in {
      view.getElementsByClass("govuk-label--xl").text() mustBe messages("whatIsTheLLPName.heading")
    }
