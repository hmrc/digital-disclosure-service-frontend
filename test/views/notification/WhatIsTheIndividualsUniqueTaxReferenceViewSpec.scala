...
class WhatIsTheIndividualsUniqueTaxReferenceViewSpec extends ViewSpecBase with ViewMatchers {
  ...
  "view" should {
    ...
    "have title" in {
      view.select("title").text() must include(messages("whatIsTheIndividualsUniqueTaxReference.title"))
    }
    ...
    "have a hint" in {
      view.getElementsByClass("govuk-hint").text() mustBe messages("whatIsTheIndividualsUniqueTaxReference.hint")
    }
