...
class IsTheIndividualRegisteredForVATViewSpec extends ViewSpecBase with ViewMatchers {
  ...
  "view" should {
    ...
    "have title" in {
      view.select("title").text() must include(messages("isTheIndividualRegisteredForVAT.title"))
    }
    ...
    "have No" in {
      view.getElementsByClass("govuk-radios__label").get(2).text() mustBe messages("isTheIndividualRegisteredForVAT.no")
    }
