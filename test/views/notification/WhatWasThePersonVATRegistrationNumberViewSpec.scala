...
class WhatWasThePersonVATRegistrationNumberViewSpec extends ViewSpecBase with ViewMatchers {
  ...
  "view" should {
    ...
    "have title" in {
      view.select("title").text() must include(messages("whatWasThePersonVATRegistrationNumber.title"))
    }
    ...
    "have a label" in {
      view.getElementsByClass("govuk-label").text() mustBe messages("whatWasThePersonVATRegistrationNumber.heading")
    }

    "have a body" in {
      view.getElementById("label").text() mustBe messages("whatWasThePersonVATRegistrationNumber.body")
    }

    "have a hint" in {
      view.getElementsByClass("govuk-hint").text() mustBe messages("whatWasThePersonVATRegistrationNumber.hint")
    }
