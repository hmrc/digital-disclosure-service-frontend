  ...
  "Reason Navigator" - {
    ...
    "in Normal mode" - {
      ...
      "must go from WhatIsTheReasonForMakingADisclosureNowPage to WhyNotBeforeNowController" in {
        navigator.nextPage(WhatIsTheReasonForMakingADisclosureNowPage, NormalMode, UserAnswers("id", "session-123")) mustBe routes.WhyNotBeforeNowController.onPageLoad(NormalMode)
      }

      "must go from WhyNotBeforeNowPage to DidSomeoneGiveYouAdviceNotDeclareTaxController" in {
        navigator.nextPage(WhyNotBeforeNowPage, NormalMode, UserAnswers("id", "session-123")) mustBe routes.DidSomeoneGiveYouAdviceNotDeclareTaxController.onPageLoad(NormalMode)
      }
      ...
      "must go from PersonWhoGaveAdvicePage to AdviceBusinessesOrOrgController" in {
        navigator.nextPage(PersonWhoGaveAdvicePage, NormalMode, UserAnswers("id", "session-123")) mustBe routes.AdviceBusinessesOrOrgController.onPageLoad(NormalMode)
      }
      ...
      "must go from AdviceBusinessNamePage to AdviceProfessionController" in {
        navigator.nextPage(AdviceBusinessNamePage, NormalMode, UserAnswers("id", "session-123")) mustBe routes.AdviceProfessionController.onPageLoad(NormalMode)
      }

      "must go from AdviceProfessionPage to AdviceGivenController" in {
        navigator.nextPage(AdviceProfessionPage, NormalMode, UserAnswers("id", "session-123")) mustBe routes.AdviceGivenController.onPageLoad(NormalMode)
      }

      "if the user says yes must go from WhichTelephoneNumberCanWeContactYouWithPage to CheckYourAnswersController" in {
        val ua = UserAnswers("id", "session-123").set(WhichTelephoneNumberCanWeContactYouWithPage, WhichTelephoneNumberCanWeContactYouWith.ExistingNumber).success.value
      ...
      "if the user says no must go from WhichTelephoneNumberCanWeContactYouWithPage to WhichEmailAddressCanWeContactYouWithController" in {
        val ua = UserAnswers("id", "session-123").set(WhichTelephoneNumberCanWeContactYouWithPage, WhichTelephoneNumberCanWeContactYouWith.DifferentNumber).success.value
      ...
      "if the user select existing email radio button must go from WhichEmailAddressCanWeContactYouWithPage to CheckYourAnswersController" in {
        val ua = UserAnswers("id", "session-123").set(WhichEmailAddressCanWeContactYouWithPage, WhichEmailAddressCanWeContactYouWith.ExistingEmail).success.value
      ...
      "if the user select different email radio button must go from WhichEmailAddressCanWeContactYouWithPage to WhatEmailAddressCanWeContactYouWithPage" in {
        val ua = UserAnswers("id", "session-123").set(WhichEmailAddressCanWeContactYouWithPage, WhichEmailAddressCanWeContactYouWith.DifferentEmail).success.value
      ...
      "must go from WhatEmailAddressCanWeContactYouWithPage to CheckYourAnswersController" in {
        navigator.nextPage(WhatEmailAddressCanWeContactYouWithPage, NormalMode, UserAnswers("id", "session-123")) mustBe routes.CheckYourAnswersController.onPageLoad
      }

      "must go from WhatTelephoneNumberCanWeContactYouWithPage to CheckYourAnswersController" in {
        navigator.nextPage(WhatTelephoneNumberCanWeContactYouWithPage, NormalMode, UserAnswers("id", "session-123")) mustBe routes.CheckYourAnswersController.onPageLoad
      }
    ...
    "in Check mode" - {
      ...
      "must go from the DidSomeoneGiveYouAdviceNotDeclareTaxPage to CheckYourAnswers when the answer is No and has changed" in {
        val userAnswers = UserAnswers("id", "session-123").set(DidSomeoneGiveYouAdviceNotDeclareTaxPage, false).success.value
      ...
      "must go from the DidSomeoneGiveYouAdviceNotDeclareTaxPage to CheckYourAnswers when the answer is Yes and has not changed" in {
        val userAnswers = UserAnswers("id", "session-123").set(DidSomeoneGiveYouAdviceNotDeclareTaxPage, true).success.value
