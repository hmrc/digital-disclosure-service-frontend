  ...
  "Reason Navigator" - {
    ...
    "in Normal mode" - {
      ...
      "must go from WhyAreYouMakingADisclosurePage to WhatIsTheReasonForMakingADisclosureNowController when selected Other" in {
        ...
        val userAnswers                          = UserAnswers("id", "session-123").set(WhyAreYouMakingADisclosurePage, set).success.value
        navigator.nextPage(
          WhyAreYouMakingADisclosurePage,
          NormalMode,
          userAnswers
        ) mustBe routes.WhatIsTheReasonForMakingADisclosureNowController.onPageLoad(NormalMode)
      }
      ...
      Seq(
        ...
        s"must go from WhyAreYouMakingADisclosurePage to WhyNotBeforeNowController when selected option $option" in {
          ...
          navigator.nextPage(
            ...
            NormalMode,
            userAnswers
          ) mustBe routes.WhyNotBeforeNowController.onPageLoad(NormalMode)
        }
      }
      ...
      "must go from WhyNotBeforeNowPage to DidSomeoneGiveYouAdviceNotDeclareTaxController" in {
        navigator.nextPage(
          ...
          NormalMode,
          UserAnswers("id", "session-123")
        ) mustBe routes.DidSomeoneGiveYouAdviceNotDeclareTaxController.onPageLoad(NormalMode)
      }

      "if the user says yes must go from DidSomeoneGiveYouAdviceNotDeclareTaxPage to PersonWhoGaveAdviceController" in {
        val ua = UserAnswers("id", "session-123").set(DidSomeoneGiveYouAdviceNotDeclareTaxPage, true).success.value
        navigator.nextPage(
          DidSomeoneGiveYouAdviceNotDeclareTaxPage,
          NormalMode,
          ua
        ) mustBe routes.PersonWhoGaveAdviceController.onPageLoad(NormalMode)
      }

      "if the user says no must go from DidSomeoneGiveYouAdviceNotDeclareTaxPage to Summary page" in {
        val ua = UserAnswers("id", "session-123").set(DidSomeoneGiveYouAdviceNotDeclareTaxPage, false).success.value
        navigator.nextPage(
          DidSomeoneGiveYouAdviceNotDeclareTaxPage,
          NormalMode,
          ua
      ...
      "must go from PersonWhoGaveAdvicePage to AdviceBusinessesOrOrgController" in {
        navigator.nextPage(
          ...
          NormalMode,
          UserAnswers("id", "session-123")
        ) mustBe routes.AdviceBusinessesOrOrgController.onPageLoad(NormalMode)
      }

      "if the user says yes must go from AdviceBusinessesOrOrgPage to AdviceBusinessNameController" in {
        val ua = UserAnswers("id", "session-123").set(AdviceBusinessesOrOrgPage, true).success.value
        navigator.nextPage(AdviceBusinessesOrOrgPage, NormalMode, ua) mustBe routes.AdviceBusinessNameController
          .onPageLoad(NormalMode)
      }

      "if the user says no must go from AdviceBusinessesOrOrgPage to AdviceProfessionController" in {
        val ua = UserAnswers("id", "session-123").set(AdviceBusinessesOrOrgPage, false).success.value
        navigator.nextPage(AdviceBusinessesOrOrgPage, NormalMode, ua) mustBe routes.AdviceProfessionController
          .onPageLoad(NormalMode)
      }
      ...
      "must go from AdviceBusinessNamePage to AdviceProfessionController" in {
        navigator.nextPage(
          ...
          NormalMode,
          UserAnswers("id", "session-123")
        ) mustBe routes.AdviceProfessionController.onPageLoad(NormalMode)
      }
      ...
      "must go from AdviceProfessionPage to AdviceGivenController" in {
        navigator.nextPage(
          ...
          NormalMode,
          UserAnswers("id", "session-123")
        ) mustBe routes.AdviceGivenController.onPageLoad(NormalMode)
      }
      ...
      "if the user says yes must go from WhichTelephoneNumberCanWeContactYouWithPage to CheckYourAnswersController" in {
        val ua = UserAnswers("id", "session-123")
          .set(WhichTelephoneNumberCanWeContactYouWithPage, WhichTelephoneNumberCanWeContactYouWith.ExistingNumber)
          .success
          .value
        navigator.nextPage(
      ...
      "if the user says no must go from WhichTelephoneNumberCanWeContactYouWithPage to WhichEmailAddressCanWeContactYouWithController" in {
        val ua = UserAnswers("id", "session-123")
          .set(WhichTelephoneNumberCanWeContactYouWithPage, WhichTelephoneNumberCanWeContactYouWith.DifferentNumber)
          .success
          .value
        navigator.nextPage(
          WhichTelephoneNumberCanWeContactYouWithPage,
          NormalMode,
          ua
        ) mustBe routes.WhatTelephoneNumberCanWeContactYouWithController.onPageLoad(NormalMode)
      }
      ...
      "if the user select existing email radio button must go from WhichEmailAddressCanWeContactYouWithPage to CheckYourAnswersController" in {
        val ua = UserAnswers("id", "session-123")
          .set(WhichEmailAddressCanWeContactYouWithPage, WhichEmailAddressCanWeContactYouWith.ExistingEmail)
          .success
          .value
        navigator.nextPage(
      ...
      "if the user select different email radio button must go from WhichEmailAddressCanWeContactYouWithPage to WhatEmailAddressCanWeContactYouWithPage" in {
        val ua = UserAnswers("id", "session-123")
          .set(WhichEmailAddressCanWeContactYouWithPage, WhichEmailAddressCanWeContactYouWith.DifferentEmail)
          .success
          .value
        navigator.nextPage(
          WhichEmailAddressCanWeContactYouWithPage,
          NormalMode,
          ua
        ) mustBe routes.WhatEmailAddressCanWeContactYouWithController.onPageLoad(NormalMode)
      }
      ...
      "must go from AdviceGivenPage to WhichEmailAddressCanWeContactYouWithController if the user already entered an email" in {
        ...
        val adviceUa    = UserAnswers("id", "session-123").set(AdviceGivenPage, adviceGiven).success.value
        navigator.nextPage(
          AdviceGivenPage,
          NormalMode,
          adviceUa
        ) mustBe routes.WhichEmailAddressCanWeContactYouWithController.onPageLoad(NormalMode)
      }
      ...
      "must go from AdviceGivenPage to WhichTelephoneNumberCanWeContactYouWithController if the user already entered a telephone number" in {
        ...
        navigator.nextPage(
          ...
          NormalMode,
          adviceUa
        ) mustBe routes.WhichTelephoneNumberCanWeContactYouWithController.onPageLoad(NormalMode)
      }
    ...
    "in Check mode" - {
      ...
      "must go from the WhyAreYouMakingADisclosurePage to WhatIsTheReasonForMakingADisclosureNowController when user select 'Other'" in {
        ...
        navigator.nextPage(
          ...
          userAnswers,
          true
        ) mustBe routes.WhatIsTheReasonForMakingADisclosureNowController.onPageLoad(CheckMode)
      }
      ...
      "must go from the DidSomeoneGiveYouAdviceNotDeclareTaxPage to CheckYourAnswers when the answer is No and has changed" in {
        val userAnswers =
          UserAnswers("id", "session-123").set(DidSomeoneGiveYouAdviceNotDeclareTaxPage, false).success.value
        navigator.nextPage(
          DidSomeoneGiveYouAdviceNotDeclareTaxPage,
          CheckMode,
          userAnswers,
      ...
      "must go from the DidSomeoneGiveYouAdviceNotDeclareTaxPage to CheckYourAnswers when the answer is Yes and has not changed" in {
        val userAnswers =
          UserAnswers("id", "session-123").set(DidSomeoneGiveYouAdviceNotDeclareTaxPage, true).success.value
        navigator.nextPage(
          DidSomeoneGiveYouAdviceNotDeclareTaxPage,
          CheckMode,
          userAnswers,
      ...
      "must go from the AdviceBusinessesOrOrgPage to AdviceBusinessNameController where the answer is Yes" in {
        val userAnswers = UserAnswers("id", "session-123").set(AdviceBusinessesOrOrgPage, true).success.value
        navigator.nextPage(
          AdviceBusinessesOrOrgPage,
          CheckMode,
          userAnswers,
          true
        ) mustBe routes.AdviceBusinessNameController.onPageLoad(CheckMode)
      }

      "must go from the AdviceBusinessesOrOrgPage to CheckYourAnswers where the answer is No" in {
        val userAnswers = UserAnswers("id", "session-123").set(AdviceBusinessesOrOrgPage, false).success.value
        navigator.nextPage(
          AdviceBusinessesOrOrgPage,
          CheckMode,
          userAnswers,
      ...
      "must go from the AdviceGivenPage to WhichEmailAddressCanWeContactYouWithController where the answer is Yes, by email" in {
        val adviceGiven = AdviceGiven("Advice", MonthYear(1, 1960), AdviceContactPreference.Email)
        val userAnswers = UserAnswers("id", "session-123").set(AdviceGivenPage, adviceGiven).success.value
        navigator.nextPage(
          AdviceGivenPage,
          CheckMode,
          userAnswers,
          true
        ) mustBe routes.WhichEmailAddressCanWeContactYouWithController.onPageLoad(CheckMode)
      }
      ...
      "must go from the AdviceGivenPage to WhichTelephoneNumberCanWeContactYouWithController where the answer is Yes, by telephone" in {
        ...
        navigator.nextPage(
          ...
          userAnswers,
          true
        ) mustBe routes.WhichTelephoneNumberCanWeContactYouWithController.onPageLoad(CheckMode)
      }
