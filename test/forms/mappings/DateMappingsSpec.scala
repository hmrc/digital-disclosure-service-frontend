...
class DateMappingsSpec
  ...
  val invalidDay: Gen[Int]      = intsAboveValue(31)
  ...
  "must fail to bind a date with a day above the max" in {

    forAll(validData -> "valid date", invalidDay -> "invalid day") { (date, day) =>
      val data = Map(
      ...
      result.errors must contain(
        FormError("value.day", "error.invalidDay", List.empty)
      )
    }
  }
  ...
  "must fail to bind a date with a month above the max" in {
    ...
    forAll(validData -> "valid data", invalidMonth -> "invalid month") { (date, month) =>
      val data = Map(
        "value.day"   -> date.getDayOfMonth.toString,
      ...
      result.errors must contain(
        FormError("value.month", "error.invalidMonth", List.empty)
      )
    }
  }
  ...
  "must fail to bind a date with a missing day and month" in {
    ...
    forAll(validData -> "valid date", missingField -> "missing day", missingField -> "missing month") {
      (date, dayOpt, monthOpt) =>
        ...
        result.errors must contain(
          FormError("value.day", List("error.required.day.month"))
        )
    }
  }
  ...
  "must fail to bind a date with a missing day and year" in {
    ...
    forAll(validData -> "valid date", missingField -> "missing day", missingField -> "missing year") {
      (date, dayOpt, yearOpt) =>
        ...
        result.errors must contain(
          FormError("value.day", List("error.required.day.year"))
        )
    }
  }
  ...
  "must fail to bind a date with a missing month and year" in {
    ...
    forAll(validData -> "valid date", missingField -> "missing month", missingField -> "missing year") {
      (date, monthOpt, yearOpt) =>
        ...
        result.errors must contain(
          FormError("value.month", List("error.required.month.year"))
        )
    }
  }
