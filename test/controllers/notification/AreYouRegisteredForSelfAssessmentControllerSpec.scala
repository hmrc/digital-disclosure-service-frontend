...
import models.{NormalMode, AreYouRegisteredForSelfAssessment, UserAnswers}
...
class AreYouRegisteredForSelfAssessmentControllerSpec extends SpecBase with MockitoSugar {
  ...
  "AreYouRegisteredForSelfAssessment Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(AreYouRegisteredForSelfAssessment.values.head), NormalMode)(request, messages(application)).toString
      }
    }
