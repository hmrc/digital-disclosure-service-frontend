...
class WhatWasTheNameOfThePersonWhoDiedControllerSpec extends SpecBase with MockitoSugar {
  ...
  "WhatWasTheNameOfThePersonWhoDied Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, whatWasTheNameOfThePersonWhoDiedRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[WhatWasTheNameOfThePersonWhoDiedView]
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, whatWasTheNameOfThePersonWhoDiedRoute)
