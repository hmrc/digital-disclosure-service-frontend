...
import views.html.notification.WhatWasThePersonVATRegistrationNumberView

import scala.concurrent.Future

class WhatWasThePersonVATRegistrationNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val whatWasThePersonVATRegistrationNumberRoute = routes.WhatWasThePersonVATRegistrationNumberController.onPageLoad(NormalMode).url
  ...
  "WhatWasThePersonVATRegistrationNumber Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val request = FakeRequest(GET, whatWasThePersonVATRegistrationNumberRoute)

      val result = route(application, request).value

      val view = application.injector.instanceOf[WhatWasThePersonVATRegistrationNumberView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId, "session-123").set(WhatWasThePersonVATRegistrationNumberPage, "answer").success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, whatWasThePersonVATRegistrationNumberRoute)
