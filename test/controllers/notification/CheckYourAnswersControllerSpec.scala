...
class CheckYourAnswersControllerSpec extends SpecBase with SummaryListFluency {
  ...
  "Check Your Answers Controller" - {
    ...
    def rowIsDisplayedWhenPageIsPopulated(ua: UserAnswers, isTheEntity: Boolean = true)(
      summaryList: Messages => SummaryLists
    ) = {
    ...
    "must return OK and the correct view for a GET when RelatesTo (A Company) is populated" in {
      ...
      rowIsDisplayedWhenPageIsPopulated(uaWithRelatesTo)(messages =>
        SummaryLists(
          SummaryListViewModel(
            Seq(
              RelatesToSummary.row(uaWithRelatesTo)(using messages),
              LiabilitiesSummary.row(uaWithRelatesTo)(using messages)            ).flatten
          ),
          SummaryListViewModel(Seq.empty),
    ...
    "must return OK and the correct view for a GET when RelatesTo (A Limited Liability Partnership) is populated" in {
      val uaWithRelatesTo =
        UserAnswers("id", "session-123").set(RelatesToPage, RelatesTo.ALimitedLiabilityPartnership).success.value
      rowIsDisplayedWhenPageIsPopulated(uaWithRelatesTo)(messages =>
        SummaryLists(
          SummaryListViewModel(
            Seq(
    ...
    "must return OK and the correct view for a GET when AreYouRepresentingAnOrganisationPage is populated" in {
      val ua = UserAnswers("id", "session-123")
        .set(AreYouRepresentingAnOrganisationPage, arbitrary[Boolean].sample.value)
        .success
        .value
      rowIsDisplayedWhenPageIsPopulated(ua)(messages =>
        SummaryLists(
          SummaryListViewModel(
            Seq(AreYouRepresentingAnOrganisationSummary.row(ua)(using messages), LiabilitiesSummary.row(ua)(using messages)).flatten
          ),
          SummaryListViewModel(Seq.empty),
          None
        )
    ...
    "must return OK and the correct view for a GET when WhatIsTheNameOfTheOrganisationYouRepresentPage is populated" in {
      val ua = UserAnswers("id", "session-123")
        .set(WhatIsTheNameOfTheOrganisationYouRepresentPage, arbitrary[String].sample.value)
        .success
        .value
      rowIsDisplayedWhenPageIsPopulated(ua)(messages =>
        SummaryLists(
          SummaryListViewModel(
            Seq(
              ...
              LiabilitiesSummary.row(ua)(using messages)            ).flatten
          ),
          SummaryListViewModel(Seq.empty),
          None
        )
    ...
    "must return OK and the correct view for a GET when DoYouHaveNationalInsuranceNumberPage is populated" in {
      val ua = UserAnswers("id", "session-123")
        .set(DoYouHaveNationalInsuranceNumberPage, arbitrary[DoYouHaveNationalInsuranceNumber].sample.value)
        .success
        .value
      rowIsDisplayedWhenPageIsPopulated(ua)(messages =>
        SummaryLists(
    ...
    "must return OK and the correct view for a GET when WhatIsYourNationalInsuranceNumberPage is populated" in {
      val ua = UserAnswers("id", "session-123")
        .set(WhatIsYourNationalInsuranceNumberPage, arbitrary[String].sample.value)
        .success
        .value
      rowIsDisplayedWhenPageIsPopulated(ua)(messages =>
        SummaryLists(
    ...
    "must return OK and the correct view for a GET when AreYouRegisteredForVATPage is populated" in {
      val ua = UserAnswers("id", "session-123")
        .set(AreYouRegisteredForVATPage, arbitrary[AreYouRegisteredForVAT].sample.value)
        .success
        .value
      rowIsDisplayedWhenPageIsPopulated(ua)(messages =>
        SummaryLists(
    ...
    "must return OK and the correct view for a GET when WhatIsYourVATRegistrationNumberPage is populated" in {
      val ua = UserAnswers("id", "session-123")
        .set(WhatIsYourVATRegistrationNumberPage, arbitrary[String].sample.value)
        .success
        .value
      rowIsDisplayedWhenPageIsPopulated(ua)(messages =>
        SummaryLists(
    ...
    "must return OK and the correct view for a GET when AreYouRegisteredForSelfAssessmentPage is populated" in {
      val ua = UserAnswers("id", "session-123")
        .set(AreYouRegisteredForSelfAssessmentPage, arbitrary[AreYouRegisteredForSelfAssessment].sample.value)
        .success
        .value
      rowIsDisplayedWhenPageIsPopulated(ua)(messages =>
        SummaryLists(
    ...
    "must return OK and the correct view for a GET when WhatIsYourUniqueTaxReferencePage is populated" in {
      val ua = UserAnswers("id", "session-123")
        .set(WhatIsYourUniqueTaxReferencePage, arbitrary[String].sample.value)
        .success
        .value
      rowIsDisplayedWhenPageIsPopulated(ua)(messages =>
        SummaryLists(
    ...
    "must return OK and the correct view for a GET when WhatIsTheIndividualsFullNamePage is populated" in {
      val ua = (for {
        userAnswer                <-
          UserAnswers("id", "session-123").set(WhatIsTheIndividualsFullNamePage, arbitrary[String].sample.value)
        uaWithRelatesToPage       <- userAnswer.set(RelatesToPage, RelatesTo.AnIndividual)
        uaWithAreYouTheEntityPage <-
          uaWithRelatesToPage.set(AreYouTheEntityPage, AreYouTheEntity.IAmAnAccountantOrTaxAgent)
      } yield uaWithAreYouTheEntityPage).success.value

      rowIsDisplayedWhenPageIsPopulated(ua, false)(messages =>
        SummaryLists(
    ...
    "must return OK and the correct view for a GET when WhatIsTheIndividualDateOfBirthPage is populated" in {
      val ua = (for {
        userAnswer                <-
          UserAnswers("id", "session-123").set(WhatIsTheIndividualDateOfBirthPage, arbitrary[LocalDate].sample.value)
        uaWithRelatesToPage       <- userAnswer.set(RelatesToPage, RelatesTo.AnIndividual)
        uaWithAreYouTheEntityPage <-
          uaWithRelatesToPage.set(AreYouTheEntityPage, AreYouTheEntity.IAmAnAccountantOrTaxAgent)
      } yield uaWithAreYouTheEntityPage).success.value
    ...
    "must return OK and the correct view for a GET when WhatIsTheIndividualOccupationPage is populated" in {
      val ua = (for {
        userAnswer                <-
          UserAnswers("id", "session-123").set(WhatIsTheIndividualOccupationPage, arbitrary[String].sample.value)
        uaWithRelatesToPage       <- userAnswer.set(RelatesToPage, RelatesTo.AnIndividual)
        uaWithAreYouTheEntityPage <-
          uaWithRelatesToPage.set(AreYouTheEntityPage, AreYouTheEntity.IAmAnAccountantOrTaxAgent)
      } yield uaWithAreYouTheEntityPage).success.value
    ...
    "must return OK and the correct view for a GET when DoesTheIndividualHaveNationalInsuranceNumberPage is populated" in {
      val ua = (for {
        userAnswer                <- UserAnswers("id", "session-123").set(
                                       DoesTheIndividualHaveNationalInsuranceNumberPage,
                                       arbitrary[DoesTheIndividualHaveNationalInsuranceNumber].sample.value
                                     )
        uaWithRelatesToPage       <- userAnswer.set(RelatesToPage, RelatesTo.AnIndividual)
        uaWithAreYouTheEntityPage <-
          uaWithRelatesToPage.set(AreYouTheEntityPage, AreYouTheEntity.IAmAnAccountantOrTaxAgent)
      ...
      rowIsDisplayedWhenPageIsPopulated(ua, false)(messages =>
        SummaryLists(
          ...
          aboutTheIndividualList = Some(
            SummaryListViewModel(Seq(DoesTheIndividualHaveNationalInsuranceNumberSummary.row(ua)(using messages)).flatten)
          )
        )
      )
    }
    ...
    "must return OK and the correct view for a GET when WhatIsIndividualsNationalInsuranceNumberPage is populated" in {
      val ua = (for {
        userAnswer                <-
          UserAnswers("id", "session-123")
            .set(WhatIsIndividualsNationalInsuranceNumberPage, arbitrary[String].sample.value)
        uaWithRelatesToPage       <- userAnswer.set(RelatesToPage, RelatesTo.AnIndividual)
        uaWithAreYouTheEntityPage <-
          uaWithRelatesToPage.set(AreYouTheEntityPage, AreYouTheEntity.IAmAnAccountantOrTaxAgent)
      } yield uaWithAreYouTheEntityPage).success.value
      ...
      rowIsDisplayedWhenPageIsPopulated(ua, false)(messages =>
        SummaryLists(
          ...
          aboutYou = SummaryListViewModel(Seq.empty),
          aboutTheIndividualList =
            Some(SummaryListViewModel(Seq(WhatIsIndividualsNationalInsuranceNumberSummary.row(ua)(using messages)).flatten))
        )
      )
    }
    ...
    "must return OK and the correct view for a GET when IsTheIndividualRegisteredForVATPage is populated" in {
      val ua = (for {
        userAnswer                <-
          UserAnswers("id", "session-123")
            .set(IsTheIndividualRegisteredForVATPage, arbitrary[IsTheIndividualRegisteredForVAT].sample.value)
        uaWithRelatesToPage       <- userAnswer.set(RelatesToPage, RelatesTo.AnIndividual)
        uaWithAreYouTheEntityPage <-
          uaWithRelatesToPage.set(AreYouTheEntityPage, AreYouTheEntity.IAmAnAccountantOrTaxAgent)
      } yield uaWithAreYouTheEntityPage).success.value
    ...
    "must return OK and the correct view for a GET when WhatIsTheIndividualsVATRegistrationNumberPage is populated" in {
      val ua = (for {
        userAnswer                <-
          UserAnswers("id", "session-123")
            .set(WhatIsTheIndividualsVATRegistrationNumberPage, arbitrary[String].sample.value)
        uaWithRelatesToPage       <- userAnswer.set(RelatesToPage, RelatesTo.AnIndividual)
        uaWithAreYouTheEntityPage <-
          uaWithRelatesToPage.set(AreYouTheEntityPage, AreYouTheEntity.IAmAnAccountantOrTaxAgent)
      } yield uaWithAreYouTheEntityPage).success.value
      ...
      rowIsDisplayedWhenPageIsPopulated(ua, false)(messages =>
        SummaryLists(
          ...
          aboutYou = SummaryListViewModel(Seq.empty),
          aboutTheIndividualList =
            Some(SummaryListViewModel(Seq(WhatIsTheIndividualsVATRegistrationNumberSummary.row(ua)(using messages)).flatten))
        )
      )
    }
    ...
    "must return OK and the correct view for a GET when IsTheIndividualRegisteredForSelfAssessmentPage is populated" in {
      val ua = (for {
        userAnswer                <- UserAnswers("id", "session-123").set(
                                       ...
                                       arbitrary[IsTheIndividualRegisteredForSelfAssessment].sample.value
                                     )
        uaWithRelatesToPage       <- userAnswer.set(RelatesToPage, RelatesTo.AnIndividual)
        uaWithAreYouTheEntityPage <-
          uaWithRelatesToPage.set(AreYouTheEntityPage, AreYouTheEntity.IAmAnAccountantOrTaxAgent)
      ...
      rowIsDisplayedWhenPageIsPopulated(ua, false)(messages =>
        SummaryLists(
          ...
          aboutYou = SummaryListViewModel(Seq.empty),
          aboutTheIndividualList =
            Some(SummaryListViewModel(Seq(IsTheIndividualRegisteredForSelfAssessmentSummary.row(ua)(using messages)).flatten))
        )
      )
    }
    ...
    "must return OK and the correct view for a GET when WhatIsTheIndividualsUniqueTaxReferencePage is populated" in {
      val ua = (for {
        userAnswer                <-
          UserAnswers("id", "session-123")
            .set(WhatIsTheIndividualsUniqueTaxReferencePage, arbitrary[String].sample.value)
        uaWithRelatesToPage       <- userAnswer.set(RelatesToPage, RelatesTo.AnIndividual)
        uaWithAreYouTheEntityPage <-
          uaWithRelatesToPage.set(AreYouTheEntityPage, AreYouTheEntity.IAmAnAccountantOrTaxAgent)
      } yield uaWithAreYouTheEntityPage).success.value
      ...
      rowIsDisplayedWhenPageIsPopulated(ua, false)(messages =>
        SummaryLists(
          ...
          aboutYou = SummaryListViewModel(Seq.empty),
          aboutTheIndividualList =
            Some(SummaryListViewModel(Seq(WhatIsTheIndividualsUniqueTaxReferenceSummary.row(ua)(using messages)).flatten))
        )
      )
    }
    ...
    "must return OK and the correct view for a GET when WhatIsTheNameOfTheCompanyTheDisclosureWillBeAboutPage is populated" in {
      val ua = (for {
        userAnswer          <-
          UserAnswers("id", "session-123")
            .set(WhatIsTheNameOfTheCompanyTheDisclosureWillBeAboutPage, arbitrary[String].sample.value)
      ...
      rowIsDisplayedWhenPageIsPopulated(ua)(messages =>
        SummaryLists(
          background =
            SummaryListViewModel(Seq(RelatesToSummary.row(ua)(using messages), LiabilitiesSummary.row(ua)(using messages)).flatten),
          aboutYou = SummaryListViewModel(Seq.empty),
    ...
    "must return OK and the correct view for a GET when WhatIsTheCompanyRegistrationNumberPage is populated" in {
      val ua = (for {
        userAnswer          <-
          UserAnswers("id", "session-123").set(WhatIsTheCompanyRegistrationNumberPage, arbitrary[String].sample.value)
        uaWithRelatesToPage <- userAnswer.set(RelatesToPage, RelatesTo.ACompany)
      } yield uaWithRelatesToPage).success.value
    ...
    "must return OK and the correct view for a GET when CompanyAddressLookupPage is populated" in {
      ...
      rowIsDisplayedWhenPageIsPopulated(ua)(messages =>
        SummaryLists(
          ...
          aboutYou = SummaryListViewModel(Seq.empty),
          aboutTheIndividualList = None,
          aboutTheCompanyList = Some(SummaryListViewModel(Seq(CompanyAddressLookupSummary.row(ua)(using messages)).flatten))
        )
      )
    }
    ...
    "must return OK and the correct view for a GET when WhatWasTheNameOfThePersonWhoDiedPage is populated" in {
      val ua = (for {
        userAnswer          <-
          UserAnswers("id", "session-123").set(WhatWasTheNameOfThePersonWhoDiedPage, arbitrary[String].sample.value)
    ...
    "must return OK and the correct view for a GET when WhatWasThePersonDateOfBirthPage is populated" in {
      val ua = (for {
        userAnswer          <-
          UserAnswers("id", "session-123").set(WhatWasThePersonDateOfBirthPage, arbitrary[LocalDate].sample.value)
        uaWithRelatesToPage <- userAnswer.set(RelatesToPage, RelatesTo.AnEstate)
      } yield uaWithRelatesToPage).success.value
    ...
    "must return OK and the correct view for a GET when WhatWasThePersonOccupationPage is populated" in {
      val ua = (for {
        userAnswer          <-
          UserAnswers("id", "session-123").set(WhatWasThePersonOccupationPage, arbitrary[String].sample.value)
        uaWithRelatesToPage <- userAnswer.set(RelatesToPage, RelatesTo.AnEstate)
      } yield uaWithRelatesToPage).success.value
    ...
    "must return OK and the correct view for a GET when DidThePersonHaveNINOPage is populated" in {
      val ua = (for {
        userAnswer          <-
          UserAnswers("id", "session-123").set(DidThePersonHaveNINOPage, arbitrary[DidThePersonHaveNINO].sample.value)
        uaWithRelatesToPage <- userAnswer.set(RelatesToPage, RelatesTo.AnEstate)
      } yield uaWithRelatesToPage).success.value
    ...
    "must return OK and the correct view for a GET when WasThePersonRegisteredForVATPage is populated" in {
      val ua = (for {
        userAnswer          <-
          UserAnswers("id", "session-123")
            .set(WasThePersonRegisteredForVATPage, arbitrary[WasThePersonRegisteredForVAT].sample.value)
        uaWithRelatesToPage <- userAnswer.set(RelatesToPage, RelatesTo.AnEstate)
      } yield uaWithRelatesToPage).success.value
    ...
    "must return OK and the correct view for a GET when WhatWasThePersonVATRegistrationNumberPage is populated" in {
      val ua = (for {
        userAnswer          <-
          UserAnswers("id", "session-123")
            .set(WhatWasThePersonVATRegistrationNumberPage, arbitrary[String].sample.value)
        uaWithRelatesToPage <- userAnswer.set(RelatesToPage, RelatesTo.AnEstate)
      } yield uaWithRelatesToPage).success.value
      ...
      rowIsDisplayedWhenPageIsPopulated(ua)(messages =>
        SummaryLists(
          ...
          aboutTheTrustList = None,
          aboutThePersonWhoDiedList =
            Some(SummaryListViewModel(Seq(WhatWasThePersonVATRegistrationNumberSummary.row(ua)(using messages)).flatten))
        )
      )
    }
    ...
    "must return OK and the correct view for a GET when WasThePersonRegisteredForSAPage is populated" in {
      val ua = (for {
        userAnswer          <-
          UserAnswers("id", "session-123")
            .set(WasThePersonRegisteredForSAPage, arbitrary[WasThePersonRegisteredForSA].sample.value)
        uaWithRelatesToPage <- userAnswer.set(RelatesToPage, RelatesTo.AnEstate)
      } yield uaWithRelatesToPage).success.value
