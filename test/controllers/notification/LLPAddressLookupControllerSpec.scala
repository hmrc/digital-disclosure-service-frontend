...
class LLPAddressLookupControllerSpec extends SpecBase with MockFactory with ModelGenerators {

  def addressLookupOnwardRoute = Call("GET", "http://localhost:15003/foo")
  def onwardRoute = Call("GET", "/foo")

  lazy val addressLookupRoute = controllers.notification.routes.LLPAddressLookupController.lookupAddress(NormalMode).url
  lazy val mockSessionService = mock[SessionService]

  val addressLookupService = mock[AddressLookupService]

  def mockGetLLPAddressLookupRedirect(redirectUrl: Call)(
    response: Either[Error, URL]
  ): CallHandler3[Call, HeaderCarrier, MessagesApi, EitherT[Future, Error, URL]] =
    (addressLookupService
      .getLLPAddressLookupRedirect(_: Call)(_: HeaderCarrier, _: MessagesApi))
      .expects(redirectUrl, *, *)
  ...
  "lookupAddress" - {
    ...
    "must redirect to the URL returned by the address lookup service" in {
      ...
      running(application) {
        ...
        mockGetLLPAddressLookupRedirect(notification.routes.LLPAddressLookupController.retrieveConfirmedAddress(NormalMode, None))(Right(new URL("http://localhost:15003/foo")))
    ...
    "must handle error scenarios appropriately" in {
      ...
      running(application) {
        ...
        mockGetLLPAddressLookupRedirect(notification.routes.LLPAddressLookupController.retrieveConfirmedAddress(NormalMode, None))(Left(Error("Something went wrong")))
  ...
  "retrieveConfirmedAddress" - {
    ...
    "must update sessionRepo and redirect to the correct place when an address is returned by the address lookup service" in {
      ...
      lazy val retrieveAddressRoute = controllers.notification.routes.LLPAddressLookupController.retrieveConfirmedAddress(NormalMode, Some(uuid)).url
    ...
    "must redirect to the address lookup when an id isn't entered" in {
      ...
      val application = buildApplication
      lazy val retrieveAddressRoute = controllers.notification.routes.LLPAddressLookupController.retrieveConfirmedAddress(NormalMode).url
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual controllers.notification.routes.LLPAddressLookupController.lookupAddress(NormalMode).url
      }
    }
