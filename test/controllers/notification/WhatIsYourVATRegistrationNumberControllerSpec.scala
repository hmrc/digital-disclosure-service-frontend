...
class WhatIsYourVATRegistrationNumberControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val whatIsYourVATRegistrationNumberRoute = routes.WhatIsYourVATRegistrationNumberController.onPageLoad(NormalMode).url
  ...
  "WhatIsYourVATRegistrationNumber Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId, "session-123").set(WhatIsYourVATRegistrationNumberPage, "answer").success.value
