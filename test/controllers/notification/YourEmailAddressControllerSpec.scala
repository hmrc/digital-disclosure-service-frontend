...
class YourEmailAddressControllerSpec extends SpecBase with MockitoSugar {
  ...
  "YourEmailAddress Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val request = FakeRequest(GET, yourEmailAddressRoute)

      val result = route(application, request).value

      val view = application.injector.instanceOf[YourEmailAddressView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId, "session-123").set(YourEmailAddressPage, "answer").success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      val request =
        FakeRequest(POST, yourEmailAddressRoute)
