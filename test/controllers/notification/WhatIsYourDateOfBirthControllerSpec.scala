...
class WhatIsYourDateOfBirthControllerSpec extends SpecBase with MockitoSugar {
  ...
  def postRequest(): FakeRequest[AnyContentAsFormUrlEncoded] =
    FakeRequest(POST, whatIsYourDateOfBirthRoute)
  ...
  "WhatIsYourDateOfBirth Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val view = application.injector.instanceOf[WhatIsYourDateOfBirthView]

      status(result) mustEqual OK
      contentAsString(result) mustEqual view(form, NormalMode, false)(using getRequest(), messages).toString
    }

    "must populate the view correctly on a GET when the question has previously been answered" in {

      val userAnswers =
        UserAnswers(userAnswersId, "session-123").set(WhatIsYourDateOfBirthPage, validAnswer).success.value
