...
import pages.WhatIsYourFullNamePage
...
class WhatIsYourFullNameControllerSpec extends SpecBase with MockitoSugar {
  ...
  "WhatIsYourFullName Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, whatIsYourFullNameRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[WhatIsYourFullNameView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId, "session-123").set(WhatIsYourFullNamePage, "answer").success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, whatIsYourFullNameRoute)
