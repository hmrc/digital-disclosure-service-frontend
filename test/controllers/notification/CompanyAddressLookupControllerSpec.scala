...
class CompanyAddressLookupControllerSpec extends SpecBase with MockFactory with ModelGenerators {
  ...
  lazy val addressLookupRoute = notification.routes.CompanyAddressLookupController.lookupAddress(NormalMode).url
  ...
  "retrieveConfirmedAddress" - {
    ...
    "must update sessionRepo and redirect to the correct place when an address is returned by the address lookup service" in {
      ...
      lazy val retrieveAddressRoute = notification.routes.CompanyAddressLookupController.retrieveConfirmedAddress(NormalMode, Some(uuid)).url
    ...
    "must redirect to the address lookup when an id isn't entered" in {
      ...
      val application = buildApplication
      lazy val retrieveAddressRoute = notification.routes.CompanyAddressLookupController.retrieveConfirmedAddress(NormalMode).url
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual notification.routes.CompanyAddressLookupController.lookupAddress(NormalMode).url
      }
    }
