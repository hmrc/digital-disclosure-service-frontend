...
class IndividualAddressLookupControllerSpec extends SpecBase with MockFactory with ModelGenerators {
  ...
  lazy val addressLookupRoute = controllers.notification.routes.IndividualAddressLookupController.lookupAddress(NormalMode).url
  ...
  def mockGetIndividualAddressLookupRedirect(redirectUrl: Call)(
    ...
    (addressLookupService
      .getIndividualAddressLookupRedirect(_: Call)(_: HeaderCarrier, _: MessagesApi))
      .expects(redirectUrl, *, *)
  ...
  "lookupAddress" - {
    ...
    "must redirect to the URL returned by the address lookup service" in {
      ...
      running(application) {
        ...
        mockGetIndividualAddressLookupRedirect(notification.routes.IndividualAddressLookupController.retrieveConfirmedAddress(NormalMode, None))(Right(new URL("http://localhost:15003/foo")))
  ...
  "retrieveConfirmedAddress" - {
    ...
    "must update sessionRepo and redirect to the correct place when an address is returned by the address lookup service" in {
      ...
      lazy val retrieveAddressRoute = controllers.notification.routes.IndividualAddressLookupController.retrieveConfirmedAddress(NormalMode, Some(uuid)).url
    ...
    "must redirect to the address lookup when an id isn't entered" in {
      ...
      val application = buildApplication
      lazy val retrieveAddressRoute = controllers.notification.routes.IndividualAddressLookupController.retrieveConfirmedAddress(NormalMode).url
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual controllers.notification.routes.IndividualAddressLookupController.lookupAddress(NormalMode).url
      }
    }
