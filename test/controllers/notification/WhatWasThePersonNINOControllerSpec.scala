...
class WhatWasThePersonNINOControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val whatWasThePersonNINORoute = controllers.notification.routes.WhatWasThePersonNINOController.onPageLoad(NormalMode).url

  "WhatWasThePersonNINO Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, whatWasThePersonNINORoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[WhatWasThePersonNINOView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId, "session-123").set(WhatWasThePersonNINOPage, "answer").success.value
