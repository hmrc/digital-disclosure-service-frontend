...
class LetterReferenceControllerSpec extends SpecBase with MockitoSugar {
  ...
  "LetterReference Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      val request = FakeRequest(GET, letterReferenceRoute)

      val result = route(application, request).value

      val view = application.injector.instanceOf[LetterReferenceView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId, "session-123").set(LetterReferencePage, "answer").success.value
