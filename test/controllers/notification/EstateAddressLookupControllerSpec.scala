...
class EstateAddressLookupControllerSpec extends SpecBase with ModelGenerators {
  ...
  def mockGetEstateAddressLookupRedirect(
    redirectUrl: Call
  )(response: Either[Error, URL]): OngoingStubbing[EitherT[Future, Error, URL]] =
    when(mockAddressLookupService.getEstateAddressLookupRedirect(eqTo(redirectUrl))(using any(), any()))
  ...
  "lookupAddress" - {
    ...
    "must redirect to the URL returned by the address lookup service" in {
      ...
      mockGetEstateAddressLookupRedirect(
        routes.EstateAddressLookupController.retrieveConfirmedAddress(NormalMode, None)
  ...
  "retrieveConfirmedAddress" - {
    ...
    "must update sessionRepo and redirect to the correct place when an address is returned by the address lookup service" in {
      ...
      lazy val retrieveAddressRoute =
        routes.EstateAddressLookupController.retrieveConfirmedAddress(NormalMode, Some(uuid)).url
