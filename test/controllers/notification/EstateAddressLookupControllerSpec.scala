...
class EstateAddressLookupControllerSpec extends SpecBase with ModelGenerators {

  def addressLookupOnwardRoute = Call("GET", "http://localhost:15003/foo")
  def onwardRoute = Call("GET", "/foo")

  lazy val addressLookupRoute = routes.EstateAddressLookupController.lookupAddress(NormalMode).url
  ...
  def mockGetEstateAddressLookupRedirect(redirectUrl: Call)
    ...
    when(mockAddressLookupService.getEstateAddressLookupRedirect(eqTo(redirectUrl))(any(), any()))
  ...
  "lookupAddress" - {
    ...
    "must redirect to the URL returned by the address lookup service" in {
      ...
      mockGetEstateAddressLookupRedirect(routes.EstateAddressLookupController.retrieveConfirmedAddress(NormalMode, None))(Right(new URL("http://localhost:15003/foo")))
    ...
    "must handle error scenarios appropriately" in {
      ...
      mockGetEstateAddressLookupRedirect(routes.EstateAddressLookupController.retrieveConfirmedAddress(NormalMode, None))(Left(Error("Something went wrong")))
  ...
  "retrieveConfirmedAddress" - {
    ...
    "must update sessionRepo and redirect to the correct place when an address is returned by the address lookup service" in {
      ...
      lazy val retrieveAddressRoute = routes.EstateAddressLookupController.retrieveConfirmedAddress(NormalMode, Some(uuid)).url
    ...
    "must redirect to the address lookup when an id isn't entered" in {
      ...
      lazy val retrieveAddressRoute = routes.EstateAddressLookupController.retrieveConfirmedAddress(NormalMode).url
      ...
      status(result) mustEqual SEE_OTHER
      redirectLocation(result).value mustEqual routes.EstateAddressLookupController.lookupAddress(NormalMode).url
    }
