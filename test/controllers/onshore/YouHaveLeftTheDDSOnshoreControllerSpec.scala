...
class YouHaveLeftTheDDSOnshoreControllerSpec extends SpecBase with MockitoSugar {
  ...
  "YouHaveLeftTheDDSOnshore Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      contentAsString(result) mustEqual view(form, NormalMode)(using request, messages, config).toString
    }

    "must populate the view correctly on a GET when the question has previously been answered" in {

      val userAnswers =
        UserAnswers(userAnswersId, "session-123").set(YouHaveLeftTheDDSOnshorePage, "answer").success.value
      ...
      status(result) mustEqual OK
      contentAsString(result) mustEqual view(form.fill("answer"), NormalMode)(using request, messages, config).toString
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      contentAsString(result) mustEqual view(boundForm, NormalMode)(using request, messages, config).toString
    }
