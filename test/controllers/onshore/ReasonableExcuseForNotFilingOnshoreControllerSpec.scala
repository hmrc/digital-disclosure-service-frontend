...
import models.{AreYouTheEntity, NormalMode, ReasonableExcuseForNotFilingOnshore, RelatesTo, UserAnswers, SubmissionType}
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = (for {
        ua <- UserAnswers(userAnswersId, "session-123", "submission-123", SubmissionType.Disclosure, JsObject.empty, Instant.now, Instant.now, models.store.Metadata(), false, None)
          .set(ReasonableExcuseForNotFilingOnshorePage, ReasonableExcuseForNotFilingOnshore("value 1", "value 1"))
