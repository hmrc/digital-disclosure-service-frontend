...
class DirectorLoanAccountLiabilitiesControllerSpec extends SpecBase with MockitoSugar {
  ...
  "DirectorLoanAccountLiabilities Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustEqual view(form.fill(answer), NormalMode, index, penaltyFlags)(using
        request,
        messages
      ).toString
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      contentAsString(result) mustEqual view(boundForm, NormalMode, index, penaltyFlags)(using
        request,
        messages
      ).toString
    }
