...
class AdviceBusinessesOrOrgControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val adviceBusinessesOrOrgRoute = routes.AdviceBusinessesOrOrgController.onPageLoad(NormalMode).url
  ...
  "AdviceBusinessesOrOrg Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId, "session-123").set(AdviceBusinessesOrOrgPage, true).success.value
      ...
      status(result) mustEqual OK
      contentAsString(result) mustEqual view(form.fill(true), NormalMode)(using request, messages).toString
    }
