...
class CheckYourAnswersViewModelSpec extends SpecBase with ScalaCheckPropertyChecks with CurrentTaxYear {
  ...
  "penaltyAmount" - {
    ...
    "apply the penalty rate to the amount of unpaid tax" in {
      forAll(arbitrary[OnshoreTaxYearWithLiabilities]) { taxYearWithLiabilities =>
        ...
        val unpaidTax      = BigDecimal(taxYearWithLiabilities.taxYearLiabilities.unpaidTax) + BigDecimal(
          taxYearWithLiabilities.taxYearLiabilities.niContributions
        )
  ...
  "yearTotal" - {
    ...
    "add the penalty amount to the unpaid tax and the interest" in {
      forAll(arbitrary[OnshoreTaxYearWithLiabilities]) { taxYearWithLiabilities =>
        val penaltyRate                = taxYearWithLiabilities.taxYearLiabilities.penaltyRate
        val unpaidTax: BigDecimal      = BigDecimal(taxYearWithLiabilities.taxYearLiabilities.unpaidTax) + BigDecimal(
          taxYearWithLiabilities.taxYearLiabilities.niContributions
        )
  ...
  def checkSummaryRows(summaryList: SummaryList, onshoreTaxYearLiabilities: OnshoreTaxYearWithLiabilities) = {
    ...
    summaryList.rows(0).value mustEqual ValueViewModel(
      HtmlContent(s"&pound;${onshoreTaxYearLiabilities.taxYearLiabilities.unpaidTax}")
    )
    ...
    summaryList.rows(1).value mustEqual ValueViewModel(
      HtmlContent(s"&pound;${onshoreTaxYearLiabilities.taxYearLiabilities.niContributions}")
    )
    ...
    summaryList.rows(2).value mustEqual ValueViewModel(
      HtmlContent(s"&pound;${onshoreTaxYearLiabilities.taxYearLiabilities.interest}")
    )
    ...
    summaryList.rows(3).value mustEqual ValueViewModel(
      Text(mess("site.2DP", onshoreTaxYearLiabilities.taxYearLiabilities.penaltyRate) + "%")
    )
  }
