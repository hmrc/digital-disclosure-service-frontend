...
class DirectorLoanAccountLiabilitiesSummaryViewModelSpec extends SpecBase with ScalaCheckPropertyChecks {
  ...
  "DirectorLoanAccountLiabilitiesSummaryViewModel" - {
    ...
    "return the director loan account section if user have added one or more accounting details" in {
      ...
      summaryList.rows(1).key mustEqual Key(
        Text(mess("directorLoanAccountLiabilities.periodEnd.checkYourAnswersLabel"))
      )
      summaryList.rows(1).value mustEqual ValueViewModel(
        Text(directorLoanAccountLiabilities.periodEnd.format(dateFormatter))
      )

      summaryList.rows(2).key mustEqual Key(
        Text(mess("directorLoanAccountLiabilities.overdrawn.checkYourAnswersLabel"))
      )
      summaryList.rows(2).value mustEqual ValueViewModel(HtmlContent(s"&pound;0"))

      summaryList.rows(3).key mustEqual Key(
        Text(mess("directorLoanAccountLiabilities.unpaidTax.checkYourAnswersLabel"))
      )
      ...
      summaryList.rows(5).key mustEqual Key(
        Text(mess("directorLoanAccountLiabilities.penaltyRate.checkYourAnswersLabel"))
      )
      ...
      summaryList.rows(7).key mustEqual Key(
        Text(mess("directorLoanAccountLiabilities.penaltyRateReason.checkYourAnswersLabel"))
      )
