...
import models.{AreYouTheEntity, RelatesTo, UserAnswers, WhyDidYouNotFileAReturnOnTimeOffshore}
import pages.{AreYouTheEntityPage, RelatesToPage, WhyDidYouNotFileAReturnOnTimeOffshorePage}
...
class WhyDidYouNotFileAReturnOnTimeOffshoreSummarySpec extends SpecBase {
  ...
  "WhyDidYouNotFileAReturnOnTimeOffshoreSummary.row" - {

    "must return None when the page has no answer" in {
      val userAnswers = UserAnswers("id", "session-123")
      WhyDidYouNotFileAReturnOnTimeOffshoreSummary.row(userAnswers) mustBe None
    }
