...
class AboutTheIndividualSpec extends AnyFreeSpec with Matchers with OptionValues {
  ...
  "isComplete" - {
    ...
    "must return true where they have answered necessary questions and have a VAT reg" in {
      val aboutTheIndividual = AboutTheIndividual(Some("name"), Some(LocalDate.now), Some("mainOccupation"), Some(No), None, Some(Yes), Some("VAT reg"), Some(No), None, Some(address))
      aboutTheIndividual.isComplete mustBe true
    }

    "must return true where they have answered necessary questions and have an SAUTR" in {
      val aboutTheIndividual = AboutTheIndividual(Some("name"), Some(LocalDate.now), Some("mainOccupation"), Some(No), None, Some(No), None, Some(Yes), Some("SAUTR"), Some(address))
      aboutTheIndividual.isComplete mustBe true
    }
