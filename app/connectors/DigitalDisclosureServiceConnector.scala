...
class DigitalDisclosureServiceConnectorImpl @Inject() (
  ...
  def submitNotification(notification: Notification)(implicit hc: HeaderCarrier): Future[String] =
    retry {
      httpClient
        ...
        .setHeader(AUTHORIZATION -> clientAuthToken, ACCEPT_LANGUAGE -> getLanguage)
  ...
  def generateNotificationPDF(notification: Notification)(implicit hc: HeaderCarrier): Future[ByteString] =
    ws
      ...
      .withHttpHeaders(AUTHORIZATION -> clientAuthToken, ACCEPT_LANGUAGE -> getLanguage)
      .post(Json.toJson(notification))
      .flatMap { response =>
        response.status match {
          case OK => Future.successful(response.bodyAsBytes)
          case _  =>
            Future.failed(DigitalDisclosureServiceConnector.UnexpectedResponseException(response.status, response.body))
        }
      }
  ...
  private def getLanguage(implicit hc: HeaderCarrier): String = {
    val cookies = hc.otherHeaders.toMap
      .get("Cookie")
      .map(cookieHeader => Cookies.fromCookieHeader(Some(cookieHeader)))
