...
import pages.WhyYouSubmittedAnInaccurateOffshoreReturnPage
...
object WhyYouSubmittedAnInaccurateReturnSummary {

  def row(userAnswers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    userAnswers.get(WhyYouSubmittedAnInaccurateOffshoreReturnPage).map { answers =>
            ...
            answers
              .map { answer =>
                HtmlFormat
                  .escape(
                    messages(
                      if (areTheyTheIndividual) s"WhyYouSubmittedAnInaccurateReturn.you.$answer"
                      else s"WhyYouSubmittedAnInaccurateReturn.notYou.$answer"
                    )
                  )
                  .toString
              }
      ...
      SummaryListRowViewModel(
        ...
        value = value,
        actions = Seq(
          ActionItemViewModel("site.change", routes.WhyYouSubmittedAnInaccurateReturnController.onPageLoad(CheckMode).url)
