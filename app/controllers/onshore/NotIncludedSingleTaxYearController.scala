...
import models.{Mode, OnshoreYearStarting, UserAnswers}
...
class NotIncludedSingleTaxYearController @Inject() (
  ...
  private def onMissingYearData(userAnswers: UserAnswers, mode: Mode)(
    f: (UserAnswers, Mode, OnshoreYearStarting, OnshoreYearStarting, OnshoreYearStarting) => Future[Result]
  ): Future[Result] = {
    val result = for {
    ...
    result.getOrElse(
      Future.successful(Redirect(controllers.onshore.routes.WhichOnshoreYearsController.onPageLoad(mode).url))
    )
  }
