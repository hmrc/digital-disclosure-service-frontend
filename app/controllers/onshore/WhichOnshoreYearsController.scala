...
class WhichOnshoreYearsController @Inject() (
      ...
      form
        ...
        .fold(
          formWithErrors =>
            Future.successful(
              BadRequest(
                view(formWithErrors, mode, populateChecklist(request.userAnswers), isCTCompleted, isDLCompleted)
              )
            ),
          value => {
            ...
            } yield Redirect(navigator.nextPage(WhichOnshoreYearsPage, mode, filteredAnswers, hasValueChanged))
          }
        )
  }
  ...
  def changedPages(userAnswers: UserAnswers, newValue: Set[OnshoreYears]): (List[QuestionPage[_]], Boolean) = {
    ...
    val missingYearPageList =
      if (hasChanged) List(NotIncludedSingleTaxYearPage, NotIncludedMultipleTaxYearsPage) else Nil

    val priorToList =
      newValue.intersect(Set[OnshoreYears](PriorToFiveYears, PriorToThreeYears, PriorToNineteenYears)).size match {
  ...
  def updateTaxYearLiabilities(
    newTaxYears: Seq[OnshoreYearStarting],
    ua: UserAnswers,
    liabilities: Map[String, OnshoreTaxYearWithLiabilities]
  ): Try[UserAnswers] = {
