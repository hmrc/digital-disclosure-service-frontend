  ...
  override protected def refine[A](request: OptionalDataRequest[A]): Future[Either[Result, DataRequest[A]]] = {
    ...
    request.userAnswers match {
      ...
      case Some(data) if data.submissionType == SubmissionType.Disclosure && data.metadata.submissionTime.isDefined =>
        ...
        else Future.successful(Left(Redirect(routes.SubmittedController.onPageLoad(data.metadata.reference.getOrElse("-")))))
...
class DataRequiredActionEvenSubmittedImpl @Inject()(implicit val executionContext: ExecutionContext) extends DataRequiredActionEvenSubmitted {
