...
class RentalAddressLookupController @Inject()(
  ...
  def retrieveConfirmedAddress(i: Int, mode: Mode,  id: Option[UUID] = None): Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    id match {
  ...
  private def retrieveConfirmedAddressFromAddressLookupFrontend(i: Int, mode: Mode,  addressId: UUID)(implicit request: DataRequest[AnyContent]): Future[Result] = 
