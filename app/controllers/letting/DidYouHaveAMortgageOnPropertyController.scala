...
class DidYouHaveAMortgageOnPropertyController @Inject() (
  ...
  def onPageLoad(i: Int, mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      val preparedForm =
        request.userAnswers.getBySeqIndex(LettingPropertyPage, i).flatMap(_.isMortgageOnProperty) match {
          case None        => form
        ...
        .fold(
          ...
          { value =>
            ...
            for {
              ...
              navigator.nextPage(DidYouHaveAMortgageOnPropertyPage, i, mode, updatedAnswers, hasValueChanged)
            )
          }
        )
  }
  ...
  def updateLettingProperty(
    ...
    userAnswers.getBySeqIndex(LettingPropertyPage, index).flatMap(_.isMortgageOnProperty) match {
      case Some(true) if value != true   =>
        (lettingProperty.copy(typeOfMortgage = None, otherTypeOfMortgage = None), false)
