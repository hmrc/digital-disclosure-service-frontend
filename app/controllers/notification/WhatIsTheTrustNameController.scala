...
class WhatIsTheTrustNameController @Inject()(
                                        ...
                                        formProvider: WhatIsTheTrustNameFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: WhatIsTheTrustNameView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(WhatIsTheTrustNamePage) match {
        case None => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(WhatIsTheTrustNamePage, value))
            _              <- sessionService.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(WhatIsTheTrustNamePage, mode, updatedAnswers))
      )
  }
