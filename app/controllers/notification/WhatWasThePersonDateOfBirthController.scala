...
class WhatWasThePersonDateOfBirthController @Inject()(
                                        ...
                                        formProvider: WhatWasThePersonDateOfBirthFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: WhatWasThePersonDateOfBirthView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(WhatWasThePersonDateOfBirthPage) match {
        case None => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(WhatWasThePersonDateOfBirthPage, value))
            _              <- sessionService.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(WhatWasThePersonDateOfBirthPage, mode, updatedAnswers))
      )
  }
