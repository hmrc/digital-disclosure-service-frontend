...
class YourPhoneNumberController @Inject() (
  ...
  formProvider: YourPhoneNumberFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: YourPhoneNumberView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    val preparedForm = request.userAnswers.get(YourPhoneNumberPage) match {
      case None        => form
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(YourPhoneNumberPage, value))
