...
class WhatIsTheIndividualOccupationController @Inject()(
                                        ...
                                        formProvider: WhatIsTheIndividualOccupationFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: WhatIsTheIndividualOccupationView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(WhatIsTheIndividualOccupationPage) match {
        case None => form
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(WhatIsTheIndividualOccupationPage, value))
            _              <- sessionService.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(WhatIsTheIndividualOccupationPage, mode, updatedAnswers))
      )
  }
