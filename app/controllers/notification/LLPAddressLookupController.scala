...
class LLPAddressLookupController @Inject() (
  ...
  def lookupAddress(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      addressLookupService
        .getLLPAddressLookupRedirect(continueUrl)
        .fold(
          { e: Error =>
  ...
  def retrieveConfirmedAddress(mode: Mode, id: Option[UUID] = None): Action[AnyContent] =
    (identify andThen getData andThen requireData).async { implicit request =>
      id match {
        case None            => Future.successful(Redirect(routes.LLPAddressLookupController.lookupAddress(mode)))
  ...
  private def retrieveConfirmedAddressFromAddressLookupFrontend(mode: Mode, addressId: UUID)(implicit
    ...
    addressLookupService
      ...
      .fold(
        ...
        address =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(LLPAddressLookupPage, address))
            _              <- sessionService.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(LLPAddressLookupPage, mode, updatedAnswers))
      )
      .flatten
}
