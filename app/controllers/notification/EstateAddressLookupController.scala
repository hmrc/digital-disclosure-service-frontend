...
class EstateAddressLookupController @Inject() (
  ...
  def lookupAddress(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      addressLookupService
        .getEstateAddressLookupRedirect(continueUrl)
        .fold(
          { e: Error =>
  ...
  def retrieveConfirmedAddress(mode: Mode, id: Option[UUID] = None): Action[AnyContent] =
    (identify andThen getData andThen requireData).async { implicit request =>
      id match {
        case None            => Future.successful(Redirect(routes.EstateAddressLookupController.lookupAddress(mode)))
  ...
  private def retrieveConfirmedAddressFromAddressLookupFrontend(mode: Mode, addressId: UUID)(implicit
    ...
    addressLookupService
      ...
      .fold(
        ...
        address =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(EstateAddressLookupPage, address))
            _              <- sessionService.set(updatedAnswers)
          } yield Redirect(navigator.nextPage(EstateAddressLookupPage, mode, updatedAnswers))
      )
      .flatten
}
