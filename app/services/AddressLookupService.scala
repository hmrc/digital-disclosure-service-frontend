...
class AddressLookupServiceImpl @Inject()(connector: AddressLookupConnector, addressConfig: AddressLookupConfig, config: FrontendAppConfig)(implicit ec: ExecutionContext) extends AddressLookupService with AddressLookupRequestHelper {
  ...
  def getYourAddressLookupRedirect(redirectUrl: Call, isAgentIndividual: Boolean)(implicit hc: HeaderCarrier, messages: Messages): EitherT[Future, Error, URL] =  {
    val request = lookupRequestForYourAddress(config.host, redirectUrl.url, addressConfig.addressesShowLimit, isAgentIndividual)
