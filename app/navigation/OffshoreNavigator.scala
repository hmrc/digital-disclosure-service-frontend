...
class OffshoreNavigator @Inject()() {
  ...
  private val normalRoutes: Page => UserAnswers => Call = {
    ...
    case TaxBeforeFiveYearsPage => ua => (ua.get(WhichYearsPage), ua.get(CountryOfYourOffshoreLiabilityPage), ua.get(OnshoreLiabilitiesPage)) match {
      case (Some(whichYear), _, Some(onshore)) if whichYear.contains(ReasonableExcusePriorTo) && whichYear.size == 1 && onshore => routes.YouHaveNoOffshoreLiabilitiesController.onPageLoad
    ...
    case TaxBeforeSevenYearsPage => ua => (ua.get(WhichYearsPage), ua.get(CountryOfYourOffshoreLiabilityPage), ua.get(OnshoreLiabilitiesPage)) match {
      case (Some(whichYear), _, Some(onshore)) if whichYear.contains(CarelessPriorTo) && whichYear.size == 1 && onshore => routes.YouHaveNoOffshoreLiabilitiesController.onPageLoad
    ...
    case TaxBeforeNineteenYearsPage => ua => (ua.get(WhichYearsPage), ua.get(CountryOfYourOffshoreLiabilityPage), ua.get(OnshoreLiabilitiesPage)) match {
      case (Some(whichYear), _, Some(onshore)) if whichYear.contains(DeliberatePriorTo) && whichYear.size == 1 && onshore => routes.YouHaveNoOffshoreLiabilitiesController.onPageLoad
