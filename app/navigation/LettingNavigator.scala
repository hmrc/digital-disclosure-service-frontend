...
class LettingNavigator @Inject() () {
  ...
  private val normalRoutes: Page => Int => UserAnswers => Call = {
    ...
    case PropertyStoppedBeingLetOutPage =>
      i =>
        ua =>
          ua.getBySeqIndex(LettingPropertyPage, i).flatMap(_.stoppedBeingLetOut) match {
    ...
    case WasPropertyFurnishedPage =>
      i =>
        ua =>
          ua.getBySeqIndex(LettingPropertyPage, i).flatMap(_.wasFurnished) match {
    ...
    case JointlyOwnedPropertyPage =>
      i =>
        ua =>
          ua.getBySeqIndex(LettingPropertyPage, i).flatMap(_.isJointOwnership) match {
            case Some(true) =>
              routes.WhatWasThePercentageIncomeYouReceivedFromPropertyController.onPageLoad(i, NormalMode)
            case _          => routes.DidYouHaveAMortgageOnPropertyController.onPageLoad(i, NormalMode)
          }

    case WhatWasThePercentageIncomeYouReceivedFromPropertyPage =>
      i => _ => routes.DidYouHaveAMortgageOnPropertyController.onPageLoad(i, NormalMode)

    case DidYouHaveAMortgageOnPropertyPage =>
      i =>
        ua =>
          ua.getBySeqIndex(LettingPropertyPage, i).flatMap(_.isMortgageOnProperty) match {
    ...
    case WhatTypeOfMortgageDidYouHavePage =>
      i =>
        ua =>
          ua.getBySeqIndex(LettingPropertyPage, i).flatMap(_.typeOfMortgage) match {
            case Some(TypeOfMortgageDidYouHave.Other) =>
              routes.WhatWasTheTypeOfMortgageController.onPageLoad(i, NormalMode)
            case _                                    => routes.WasALettingAgentUsedToManagePropertyController.onPageLoad(i, NormalMode)
          }

    case WhatWasTheTypeOfMortgagePage =>
      i => _ => routes.WasALettingAgentUsedToManagePropertyController.onPageLoad(i, NormalMode)

    case WasALettingAgentUsedToManagePropertyPage =>
      i =>
        ua =>
          ua.getBySeqIndex(LettingPropertyPage, i).flatMap(_.wasPropertyManagerByAgent) match {
    ...
    case DidTheLettingAgentCollectRentOnYourBehalfPage =>
      i => _ => routes.CheckYourAnswersController.onPageLoad(i, NormalMode)
    ...
    case WhatTypeOfMortgageDidYouHavePage =>
      i =>
        ua =>
          hasChanged =>
            ua.getBySeqIndex(LettingPropertyPage, i).flatMap(_.typeOfMortgage) match {
              case Some(TypeOfMortgageDidYouHave.Other) if hasChanged =>
                routes.WhatWasTheTypeOfMortgageController.onPageLoad(i, CheckMode)
              case _                                                  => routes.CheckYourAnswersController.onPageLoad(i, CheckMode)
            }
    ...
    case WasALettingAgentUsedToManagePropertyPage =>
      i =>
        ua =>
          hasChanged =>
            ua.getBySeqIndex(LettingPropertyPage, i).flatMap(_.wasPropertyManagerByAgent) match {
              case Some(true) if hasChanged =>
                routes.DidTheLettingAgentCollectRentOnYourBehalfController.onPageLoad(i, CheckMode)
              case _                        => routes.CheckYourAnswersController.onPageLoad(i, CheckMode)
            }
  ...
  def nextPage(page: Page, i: Int, mode: Mode, userAnswers: UserAnswers, hasAnswerChanged: Boolean = true): Call =
    mode match {
      case NormalMode =>
